...
window.onload = function () {
  ...
  const oneBeforeLastSlideClone = initSlides[initSlides.length - 2].cloneNode(
    true
  );
  ...
  const horizontalMove = () => {
    if (
      endMouseCoord.x < startMouseCoord.x &&
      shiftCounter !== sliderLength - 3
    ) {
      shiftCounter++;
    } else if (
      endMouseCoord.x < startMouseCoord.x &&
  ...
  const verticalMove = () => {
    if (
      endMouseCoord.y < startMouseCoord.y &&
      shiftCounter !== sliderLength - 3
    ) {
      shiftCounter++;
    } else if (
  ...
  const setSlidesWidth = () => {
    slider.style.width = `${
      (sliderContainer.offsetWidth * slidesLength) / 3
    }px`;
  ...
  const setSlidesHeight = () => {
    slider.style.height = `${
      (sliderContainer.offsetHeight * slidesLength) / 3
    }px`;
  ...
  class Star {
    ...
    update() {
      ...
      if (canvas.width <= this.x - 7 * this.size || this.x + 7 * this.size <= 0)
        this.vx = -this.vx;
      if (
        canvas.height <= this.y - 7 * this.size ||
  ...
  function createStars(array) {
    ...
    isMobileDevice() == true
      ? (starsQuantity = starsQuantityOnMobile)
